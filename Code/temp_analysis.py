...
class TempAnalysis():
    ...
    def __init__(self, datapath, configpath) -> None:
        ...
        #self.statuses = config.set('MAIN', 'STATUS_COL', str(self.data.shape[1]))
    ...
    def prepare_data(self):
        ...
        print(f"Columns: {col+1}")
       # self.statuses = col
       # print(f"Statuses: {self.statuses}")
        le = LabelEncoder()
        for x in range(col):
            #print(f"Temp: {self.data[self.data.columns[x]]}")
            #if  ((self.data[self.data.columns[x]]=="OFF") | (self.data[self.data.columns[x]]== "ON")):
            if self.data[self.data.columns[x]].dtypes == 'object':
                print(f"ON/OFF: {self.data[self.data.columns[x]]}")
            ...
            # convert data values to float64 datatype
                print(f"Temperature: {self.data[self.data.columns[x]]}")
                self.data[self.data.columns[x]] = self.data[self.data.columns[x]].astype(float)
            #  sort dataset by the first column
                self.data = self.data.sort_index()
            #  print(f"Temp: {self.data[self.data.columns[x]]}")
            """   one_hot = pd.get_dummies(self.data[self.data.columns[x]], drop_first=True)
                self.data = self.data.drop(self.data.columns[x], axis=1)
                self.data = self.data.join(one_hot)
                self.data[self.data.columns[x]] = self.data[self.data.columns[x]].astype(int)
"""
        # convert data values to float64 datatype
    ...
    def calculate_corr(self):
        '''Calculates a correlation between equipment data and external data'''
